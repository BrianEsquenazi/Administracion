...
Begin VB.Form Prgtrassurf1 
   ...
   End
End
Attribute VB_Name = "Prgtrassurf1"
   ...
   Rem Proveedores
    ...
    With RstProveedor
            ...
            Do
                ...
                WDate = IIf(IsNull(RstProveedor!WDate), "", RstProveedor!WDate)
                
                If WDate = WFectraspaso Then
                
                    WProveedor = IIf(IsNull(RstProveedor!Proveedor), "", RstProveedor!Proveedor)
                    WNombre = IIf(IsNull(RstProveedor!Nombre), "", RstProveedor!Nombre)
                    WDireccion = IIf(IsNull(RstProveedor!Direccion), "", RstProveedor!Direccion)
                    WLocalidad = IIf(IsNull(RstProveedor!Localidad), "", RstProveedor!Localidad)
                    WPostal = IIf(IsNull(RstProveedor!Postal), "", RstProveedor!Postal)
                    WCuit = IIf(IsNull(RstProveedor!Cuit), "", RstProveedor!Cuit)
                    WTelefono = IIf(IsNull(RstProveedor!Telefono), "", RstProveedor!Telefono)
                    WEMail = IIf(IsNull(RstProveedor!EMail), "", RstProveedor!EMail)
                    WObservaciones = IIf(IsNull(RstProveedor!Observaciones), "", RstProveedor!Observaciones)
                    WDias = IIf(IsNull(RstProveedor!Dias), "", RstProveedor!Dias)
                    WCuenta = IIf(IsNull(RstProveedor!Cuenta), "", RstProveedor!Cuenta)
                    WIva = IIf(IsNull(RstProveedor!Iva), "", RstProveedor!Iva)
                    WTipo = IIf(IsNull(RstProveedor!Tipo), "", RstProveedor!Tipo)
                    WProvincia = IIf(IsNull(RstProveedor!Provincia), "", RstProveedor!Provincia)
                    WNombreCheque = IIf(IsNull(RstProveedor!NombreCheque), "", RstProveedor!NombreCheque)
...
Private Sub Proceso1()
   ...
   Rem cotizaciones
    ...
    With rstCotiza
            .MoveFirst
            Do
                If rstCotiza!WDate = WFectraspaso Then
    
                    WCotiza = rstCotiza!Cotiza
                    WRenglon = rstCotiza!Renglon
                    WFecha = rstCotiza!Fecha
                    WProveedor = rstCotiza!Proveedor
                    WArticulo = rstCotiza!Articulo
                    WPrecio = rstCotiza!Precio
                    WFechaord = rstCotiza!FechaOrd
                    WCondicion = rstCotiza!Condicion
                    WObservaciones = rstCotiza!Observaciones
                    WClave = rstCotiza!Clave
