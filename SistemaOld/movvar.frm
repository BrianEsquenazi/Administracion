...
Begin VB.Form PrgMovvar 
   ...
   Begin VB.ListBox Pantalla 
      Height          =   2205
      ItemData        =   "movvar.frx":0A12
      Left            =   3840
      List            =   "movvar.frx":0A19
      TabIndex        =   1
      Top             =   5880
      Visible         =   0   'False
      Width           =   6615
...
Dim WCompa1 As Double
Dim WCompa2 As Double
...
Private Sub Command1_Click()
    ...
    Sql1 = "UPDATE Movvar SET "
    Sql2 = "Observaciones = + " + "'" + "VILLA MAIPU" + "'"
    Sql3 = " Where Observaciones = " + "'" + "Descarte" + "'"
    spMovvar = Sql1 + Sql2 + Sql3
...
Private Sub Form_Activate()
    ...
    End If
    Rem OPEN_FILE_Movvar
    Rem OPEN_FILE_TERMINADO
    Rem OPEN_FILE_Articulo
End Sub

Private Sub Graba1_Click()
    If Tipomov.ListIndex = 0 Or Tipomov.ListIndex = 1 Or Tipomov.ListIndex = 2 Or Tipomov.ListIndex = 3 Then
        WClave.Text = ""
        Pass.Visible = True
        WClave.SetFocus
    End If
...
Private Sub Graba_Click()
    ...
    If Auxi <> "S" Then
        m$ = "La fecha de del movimiento varios es incorrecta"
    ...
    If Val(Codigo.Text) = 0 Then
        ...
        If rstMovvar.RecordCount > 0 Then
            With rstMovvar
                .MoveLast
                Codigo.Text = rstMovvar!Codigo + 1
            End With
            rstMovvar.Close
                Else
            Codigo.Text = "1"
    ...
    spMovvar = "Listamovvar " + "'" + Codigo.Text + "'"
    Set rstMovvar = db.OpenRecordset(spMovvar, dbOpenSnapshot, dbSQLPassThrough)
    ...
    If rstMovvar.RecordCount > 0 Then
        With rstMovvar
            ...
            Do
                If .EOF = False Then
                    Renglon = Renglon + 1
                    Auxiliar(Renglon, 1) = rstMovvar!Tipo
                    Auxiliar(Renglon, 2) = rstMovvar!Terminado
                    Auxiliar(Renglon, 3) = rstMovvar!Articulo
                    Auxiliar(Renglon, 4) = Str$(rstMovvar!Cantidad)
                    Auxiliar(Renglon, 5) = rstMovvar!Movi
                    Auxiliar(Renglon, 6) = IIf(IsNull(rstMovvar!Lote), "0", rstMovvar!Lote)
                    .MoveNext
                        Else
                    Exit Do
                End If
    ...
    spMovvar = "BorrarMovvar " + "'" + Codigo.Text + "'"
    Set rstMovvar = db.OpenRecordset(spMovvar, dbOpenDynaset, dbSQLPassThrough)
...
Private Sub Wlote_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If WTipo.Text = "M" Then
            ...
            If WEntra = "S" Then
                WCompa1 = WCanti
                WCompa2 = Val(WCantidad.Text)
                Call Redondeo(WCompa1)
                Call Redondeo(WCompa2)
                If WCompa1 >= WCompa2 Or WMovi.Text = "E" Then
                    Call Alta_Vector
                    Call Ingresa_Click
                    WTipo.SetFocus
                        Else
 ...
 Next i
    ...
    With rstEmpresa
        ...
        If .NoMatch = False Then
            PrgMovvar.Caption = "Listado de E/S de Materia Prima y Productos :  " + !Nombre
        End If
    End With
    
    Tipomov.Clear
    
    Tipomov.AddItem "Movimiento varios (entrada)"
    Tipomov.AddItem "Movimiento varios (salida)"
    Tipomov.AddItem "Guia de Traslado  (entrada)"
    Tipomov.AddItem "Guia de Traslado  (salida)"
        ...
        With rstMovvar
            ...
            Do
                If .EOF = False Then
                    ...
                    DBGrid1.Col = 0
                    DBGrid1.Text = rstMovvar!Tipo
                
                    DBGrid1.Col = 1
                    DBGrid1.Text = rstMovvar!Terminado
                    Auxi1 = rstMovvar!Terminado
                
                    DBGrid1.Col = 2
                    DBGrid1.Text = rstMovvar!Articulo
                    Auxi2 = rstMovvar!Articulo
                
                    DBGrid1.Col = 4
                    DBGrid1.Text = Pusing("###,###.##", rstMovvar!Cantidad)
                
                    DBGrid1.Col = 5
                    DBGrid1.Text = rstMovvar!Movi
                
                    DBGrid1.Col = 6
                    DBGrid1.Text = IIf(IsNull(rstMovvar!Lote), "0", rstMovvar!Lote)
                    
                    Tipomov.ListIndex = Val(rstMovvar!Tipomov)
                    Observaciones.Text = rstMovvar!Observaciones
