...
Begin VB.Form PrgFactu 
   ...
   ScaleWidth      =   11550
   Visible         =   0   'False
   Begin VB.CommandButton Command112323 
      Caption         =   "Command1"
      Height          =   855
      Left            =   2160
      TabIndex        =   84
   ...
   Begin VB.ListBox Pantalla 
      ...
      EndProperty
      Height          =   1980
      ItemData        =   "prgfactuotro.frx":0000
      Left            =   4200
      List            =   "prgfactuotro.frx":0007
      TabIndex        =   0
      Top             =   5880
      Visible         =   0   'False
      Width           =   1815
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Height          =   3855
      Left            =   120
      OleObjectBlob   =   "prgfactuotro.frx":0015
      TabIndex        =   2
      Top             =   1680
      Width           =   11415
   End
...
Private Sub Calcula_Importe()
    ...
    If ZZPorceIbCaba <> 0 Then
        If WCodIbCiudad <> 3 Then
            WImpoIbCiudad = WNeto * (ZZPorceIbCaba / 100)
            Call Redondeo(WImpoIbCiudad)
            WImpoPorceIbCiudad = ZZPorceIbCaba
        End If
            Else
...
Private Sub Command111_Click()
    ...
    Shell "NET USE LPT1: \\federiaco-fac\HP LaserJet 2420 PCL 5e /persistent:yes"
    Open "lpt1" For Output As #1
    Print #1, "sdfsdfsdfdsfsdf"
    Print #1, Chr$(12)
    Close #1
...
Private Sub Command122_Click()
    ...
    If Val(Wempresa) = 8 Then
        ...
        For Ciclo = 1 To 99
            ...
            If Auxiliar(Ciclo, 1) <> "" Then
            
                For CicloII = 5 To 27 Step 2
                
                    ZLote = Auxiliar(Ciclo, CicloII)
...
Private Sub Command112323_Click()
...
Private Sub Command33_Click()
    ...
    Call Ceros(ZZRemito, 8)

    Open "C:\REMITOTXT\" + ZZRemito + ".TXT" For Output As #1
    
    Print #1, "EMPRESA: Surfactan s.a."
    Print #1, "RTO. N° 0001-" + ZZRemito
    Print #1, ""
    Print #1, "------------------------------------------------------------------------------------------------------------"
    Print #1, "Producto             |  Cantidad    |  Unidad    |  Lote"
...
Private Sub Graba_Click()
                ...
                If Cantidad <> 0 Then
                    ...
                    If Left$(Articulo, 2) = "PT-5" Then
                        ...
                        If Val(Wempresa) = 1 Then
                            ...
                            Rem ZZRequiereMsds = 0
                            Rem ZZRequiereMsdsCada = 0
                            Rem ZZRequiereHoja = 0
                            
                            Rem ZSql = ""
                            Rem ZSql = ZSql + "Select *"
                            Rem ZSql = ZSql + " FROM ClienteEspecif"
                            Rem ZSql = ZSql + " Where ClienteEspecif.Cliente = " + "'" + Cliente.Text + "'"
                            Rem spClienteEspecif = ZSql
                            Rem Set rstClienteEspecif = db.OpenRecordset(spClienteEspecif, dbOpenSnapshot, dbSQLPassThrough)
                            Rem If rstClienteEspecif.RecordCount > 0 Then
                            Rem     ZZRequiereCertificado = IIf(IsNull(rstClienteEspecif!RequiereCertificado), "0", rstClienteEspecif!RequiereCertificado)
                            Rem     ZZRequiereMsds = IIf(IsNull(rstClienteEspecif!RequiereMsds), "0", rstClienteEspecif!RequiereMsds)
                            Rem     ZZRequiereMsdsCada = IIf(IsNull(rstClienteEspecif!RequiereMsdsCada), "0", rstClienteEspecif!RequiereMsdsCada)
                            Rem     ZZRequiereHoja = IIf(IsNull(rstClienteEspecif!RequiereHoja), "0", rstClienteEspecif!RequiereHoja)
                            Rem     rstClienteEspecif.Close
                            Rem End If
                            
                            Rem ZZBusqueda = "N"
                            Rem If ZZRequiereMsdsCada = 1 Then
                            Rem     ZZBusqueda = "S"
                            Rem         Else
                            Rem     ZSql = ""
                            Rem     ZSql = ZSql + "Select *"
                            Rem     ZSql = ZSql + " FROM Estadistica"
                            Rem     ZSql = ZSql + " Where Estadistica.Cliente = " + "'" + Cliente.Text + "'"
                            Rem     ZSql = ZSql + " and Estadistica.Articulo = " + "'" + Articulo + "'"
                            Rem     spEstadistica = ZSql
                            Rem     Set rstEstadistica = db.OpenRecordset(spEstadistica, dbOpenSnapshot, dbSQLPassThrough)
                            Rem     If rstEstadistica.RecordCount > 0 Then
                            Rem         rstEstadistica.Close
                            Rem             Else
                            Rem         ZZBusqueda = "S"
                            Rem     End If
                            Rem End If
                            
                            Rem If ZZBusqueda = "S" Then
                            Rem     If Left$(UCase(Articulo), 2) = "PT" Then
                            Rem         ZZCodArt = Articulo
                            Rem             Else
                            Rem         ZZCodArt = Mid$(Articulo, 3) + Mid$(Articulo, 6, 7)
                            Rem     End If
                            Rem
                            Rem     ZZRuta = "w:\MSDSSIS\MSDS" + ZZCodArt + ".PDF"
                            Rem     ZZEstado = Dir(ZZRuta)
                            Rem     ZZEstado = Trim(ZZEstado)
                            Rem     If ZZEstado = "" Then
                            Rem         m$ = "No hay hoja de seguridad del Producto " + ZZCodArt
                            Rem         CA% = MsgBox(m$, 0, "Emision de Facturas")
                            Rem     End If
                            Rem
                            Rem End If
                            ...
                            Rem     ZSql = ""
                            Rem     ZSql = ZSql & "Select *"
                            Rem     ZSql = ZSql & " FROM AltaCertificado"
                            Rem     ZSql = ZSql & " Where AltaCertificado.Producto = " + "'" + Articulo + "'"
                            Rem     ZSql = ZSql & " and AltaCertificado.cliente = " + "'" + Cliente.Text + "'"
                            Rem     spAltaCertificado = ZSql
                            Rem     Set rstAltaCertificado = db.OpenRecordset(spAltaCertificado, dbOpenSnapshot, dbSQLPassThrough)
                            Rem     If rstAltaCertificado.RecordCount > 0 Then
                            Rem         rstAltaCertificado.Close
                            Rem         ZZEntra = "S"
                            Rem     End If
                                
                            Rem     If ZZEntra = "N" Then
                            Rem         ZSql = ""
                            Rem         ZSql = ZSql & "Select *"
                            Rem         ZSql = ZSql & " FROM AltaCertificado"
                            Rem         ZSql = ZSql & " Where AltaCertificado.Producto = " + "'" + Articulo + "'"
                            Rem         ZSql = ZSql & " and AltaCertificado.cliente = " + "'" + "S00102" + "'"
                            Rem         spAltaCertificado = ZSql
                            Rem         Set rstAltaCertificado = db.OpenRecordset(spAltaCertificado, dbOpenSnapshot, dbSQLPassThrough)
                            Rem         If rstAltaCertificado.RecordCount > 0 Then
                            Rem             rstAltaCertificado.Close
                            ...
                            Rem         End Select
                            Rem
                            Rem         If Val(XLote(WRenglon, ZZLugar)) <> 0 Then
                            Rem
                            Rem             Auxi = Mid$(Articulo, 1, 3) + Mid$(Articulo, 6, 7)
                            Rem             ZZCambia = "N"
                            Rem             ZSql = ""
                            Rem             ZSql = ZSql & "Select *"
                            Rem             ZSql = ZSql & " FROM Laudo"
                            Rem             ZSql = ZSql & " Where Laudo.Laudo = " + "'" + XLote(WRenglon, ZZLugar) + "'"
                            Rem             ZSql = ZSql & " and Laudo.Articulo = " + "'" + Articulo + "'"
                            Rem             spLaudo = ZSql
                            Rem             Set rstLaudo = db.OpenRecordset(spLaudo, dbOpenSnapshot, dbSQLPassThrough)
                            Rem             If rstLaudo.RecordCount > 0 Then
                            Rem                 ZZCambia = "S"
                            Rem                 ZZPartiOri = Trim(rstLaudo!PartiOri)
                            Rem                 ZZRuta = "w:\" + ZZPartiOri + ".PDF"
                            Rem                 ZZEstado = Dir(ZZRuta)
                            Rem                 ZZEstado = Trim(ZZEstado)
                            Rem                 If ZZEstado = "" Then
                            Rem                     ZZEntra = "S"
                            ...
                            Rem If ZZEntra = "N" Then
                            Rem     m$ = "No hay Certificado de Analisis del Producto " + Articulo
                            Rem     CA% = MsgBox(m$, 0, "Emision de Facturas")
                            Rem End If
    ...
    Rem If Moneda.ListIndex = 0 Then
    Rem     Call Impresion
    Rem         Else
    Rem     Call ImpresionPesos
    Rem End If
                        ...
                        If XCanti1 <> 0 Or XCanti2 <> 0 Then
                            ...
                            If Moneda.ListIndex = 0 Then
                                DBGrid1.Text = Pusing("##,###,###.##", Str$(!Precio))
                                    Else
                                ZZZZPrecio = !Precio * Val(Paridad.Text)
                                Call Redondeo(ZZZZPrecio)
                                Auxi = Str$(ZZZZPrecio)
                                Auxi = Pusing("##,###,###.##", Auxi)
                                DBGrid1.Text = Auxi
                            End If
    ...
    For DA = 1 To WRenglon
        ...
        Select Case WTipopro
            ...
            Case "T"
                ...
                If rstPrecios.RecordCount > 0 Then
                    ...
                    If Moneda.ListIndex = 0 Then
                        ...
                        Precio = rstPrecios!Precio
                            Else
                        Rem DBGrid1.Text = Pusing("##,###,###.##", Str$(rstPrecios!Precio * Val(Paridad.Text)))
                        Rem Precio = rstPrecios!Precio * Val(Paridad.Text)
                        
                        ZZZZPrecio = rstPrecios!Precio * Val(Paridad.Text)
                        Precio = ZZZZPrecio
                        Call Redondeo(ZZZZPrecio)
                        Auxi = Str$(ZZZZPrecio)
                        Auxi = Pusing("##,###,###.##", Auxi)
                ...
                If Precio = 0 Then
                    m$ = "ATENCION   Articulo : " + Auxi1 + " con precio en cero"
                    Aaa% = MsgBox(m$, 0, "Ingreso de Facturas")
                End If
    ...
    If KeyAscii = 13 Then
        ...
        If rstPedido.RecordCount > 0 Then
            If rstPedido!Autorizo <> "X" Then
                ...
                If rstPedido!MarcaFactura <> 1 Then
                    rstPedido.Close
                    m$ = "EL PEDIDO NO ESTA ACTUALIZADO"
                    a% = MsgBox(m$, 0, "Actualizacion de Pedidos")
                    Pedido.SetFocus
                    Exit Sub
                End If
...
Private Sub reImpre_Click()
    ...
    If Respuesta% = 6 Then
        ...
        Rem     Call Impresion_RemitoPrueba
        Rem         Else
        Rem     Call Impresion_Remito
        Rem End If
    End If
                            ...
                            If ZZCantidad <> 0 Then
                                ...
                                For CicloCanti = 1 To ZLugarDespa
                                    If ZZConsumo > Val(ZZImpreDespa(CicloCanti, 2)) Then
                                        ...
                                        ZZImpreDespa(CicloCanti, 2) = "0"
                                            Else
                                        Dada = ZZImpreDespa(CicloCanti, 1)
...
Rem dada
  ...
  For i = 1 To 2
    ...
    If Val(Wempresa) = 1 Then
        Rem If Cliente.Text = "T00094" Then
        Rem     If Moneda.ListIndex = 0 Then
        Rem         Listado.ReportFileName = "ImpreFacturaLocalDolarNuevoT00094.rpt"
        Rem             Else
        Rem         Listado.ReportFileName = "ImpreFacturaLocalPesosNuevoT00094.rpt"
        Rem     End If
        Rem         Else
        Rem     If Moneda.ListIndex = 0 Then
        Rem         Listado.ReportFileName = "ImpreFacturaLocalDolarNuevo.rpt"
        Rem             Else
        Rem         Listado.ReportFileName = "ImpreFacturaLocalPesosNuevo.rpt"
        Rem     End If
        Rem End If
...
Sub Impresion_RemitoPrueba()
    ...
    If Cliente.Text = "U00014" And Val(Wempresa) = 1 Then
        ...
        m$ = "Recuerde enviar por email a " + "TINTORERIA ULLUM S.A." + "el archivo con la informacion del remito. El archivo a enviar es " + "C:\REMITOTXT\" + ZZRemito + ".TXT"
        a = MsgBox(m$, 0, " INFORMACION IMPORTANTE PARA " + "TINTORERIA ULLUM S.A.")
    ...
    For aaaa = 1 To 16
        ...
        If ZZCantidad <> 0 Then
            ...
            If Cliente.Text = "U00014" And Val(Wempresa) = 1 Then
                If Val(ZCantidad1) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad1);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote1
                End If
                If Val(ZCantidad2) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad2);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote2
                End If
                
                If Val(ZCantidad3) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad3);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote3
                End If
                If Val(ZCantidad4) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad4);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote4
                End If
                If Val(ZCantidad5) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad5);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote5
                End If
                If Val(ZCantidad6) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad6);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote6
                End If
                If Val(ZCantidad7) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad7);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote7
                End If
                If Val(ZCantidad8) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad8);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote8
                End If
                If Val(ZCantidad9) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad9);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote9
                End If
                If Val(ZCantidad10) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad10);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote10
                End If
                If Val(ZCantidad11) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad11);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote11
                End If
                If Val(ZCantidad12) <> 0 Then
                    Print #1, Tab(1); Left$(ZZDescripcion, 25);
                    Print #1, Tab(29); Alinea("######", ZCantidad12);
                    Print #1, Tab(45); "KG.";
                    Print #1, Tab(57); ZLote12
                End If
            End If
...
Private Sub Verifica_Lote()
    ...
    If ZVeriSedronar = "S" Then
        ...
        fechVto = Right$(Fecha.Text, 4) + Mid$(Fecha.Text, 4, 2) + Left$(Fecha.Text, 2)
                            ...
                            If Left$(UCase(Articulo), 2) = "PT" Then
                                ...
                                If Left$(Articulo, 2) = "DY" Then
                                    ...
                                    If rstLaudo.RecordCount > 0 Then
                                        ...
                                        ZSql = ZSql + "Select *"
                                        ZSql = ZSql + " FROM Guia"
                                        ZSql = ZSql + " Where Guia.Articulo = " + "'" + ZProductoDy + "'"
                                        ZSql = ZSql + " and Guia.Lote = " + "'" + WWLote + "'"
                                        ...
                                        If rstMovguia.RecordCount > 0 Then
                                            With rstMovguia
                                                .MoveFirst
                                                ZZPartiOri = Trim(rstMovguia!PartiOri)
                                                rstMovguia.Close
                                            End With
                                        End If
                                    End If
...
Private Sub Calcula_Cae()
    ...
    ' Agrego tasas de IVA 5 iva 21% 4 iva 10.5
    id = 5 ' 21%
    base_imp = ZZImpoNeto
    Importe = ZZImpoIva
...
Private Sub xxx_Click()
    ...
    Rem ZSql = ZSql + " ClienteII = Cliente"
    Rem spCtacte = ZSql
...
Stop
    ...
    ZSql = ""
    ZSql = ZSql + "UPDATE CtaCte SET "
    ZSql = ZSql + " Cliente = " + "'" + "C00142" + "'"
    ZSql = ZSql + " Where Cliente = " + "'" + "C00145" + "'"
