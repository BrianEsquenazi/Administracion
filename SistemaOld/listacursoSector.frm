...
Begin VB.Form PrgListaCursoSector 
   AutoRedraw      =   -1  'True
   Caption         =   "Listado de Cursos Realizados por Sector"
   ClientHeight    =   7110
   ClientLeft      =   2010
   ClientTop       =   735
   ClientWidth     =   8085
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Image Consulta 
         Height          =   480
         Left            =   4560
         MouseIcon       =   "listacursoSector.frx":0000
         MousePointer    =   99  'Custom
         Picture         =   "listacursoSector.frx":030A
   ...
   End
End
Attribute VB_Name = "PrgListaCursoSector"
...
Private Sub Acepta_Click()
    ...
    Listado.WindowTitle = "Listado de Cursos por Sector"
    Listado.WindowTop = 0
    ...
    ZSql = ""
    ZSql = ZSql + "UPDATE Cursadas SET "
    ZSql = ZSql + " Cursadas.Perfil = Legajo.Perfil"
    ZSql = ZSql + " From Cursadas, Legajo"
    ZSql = ZSql + " Where Cursadas.Legajo = Legajo.Codigo"
    spCursadas = ZSql
    Set rstCursadas = db.OpenRecordset(spCursadas, dbOpenSnapshot, dbSQLPassThrough)
    ...
    ZSql = ""
    ZSql = ZSql + "UPDATE Cursadas SET "
    ZSql = ZSql + " Cursadas.Sector = Tarea.Sector"
    ZSql = ZSql + " From Cursadas, Tarea"
    ZSql = ZSql + " Where Cursadas.Perfil = Tarea.Codigo"
    spCursadas = ZSql
    Set rstCursadas = db.OpenRecordset(spCursadas, dbOpenSnapshot, dbSQLPassThrough)
    ...
    Uno = "{Cursadas.Sector} in " + Desde.Text + " to " + Hasta.Text + " and "
    ...
    Select Case Tipo.ListIndex
         Case 1
            Listado.SQLQuery = "SELECT Cursadas.Codigo, Cursadas.Curso, Cursadas.Fecha, Cursadas.OrdFecha, Cursadas.Horas, Cursadas.Legajo, Cursadas.DesLegajo, Cursadas.Observaciones, Cursadas.DesSector, Cursadas.TipoCursada, Cursadas.Sector, " _
                    + "Curso.Descripcion " _
                    + "From " _
                    ...
                    + "Cursadas.TipoCursada <= 0 AND " _
                    + "Cursadas.Sector >= " + Desde.Text + " AND " _
                    + "Cursadas.Sector <= " + Hasta.Text
         Case 2
            Listado.SQLQuery = "SELECT Cursadas.Codigo, Cursadas.Curso, Cursadas.Fecha, Cursadas.OrdFecha, Cursadas.Horas, Cursadas.Legajo, Cursadas.DesLegajo, Cursadas.Observaciones, Cursadas.DesSector, Cursadas.TipoCursada, Cursadas.Sector, " _
                    + "Curso.Descripcion " _
                    + "From " _
         ...
         Case Else
            Listado.SQLQuery = "SELECT Cursadas.Codigo, Cursadas.Curso, Cursadas.Fecha, Cursadas.OrdFecha, Cursadas.Horas, Cursadas.Legajo, Cursadas.DesLegajo, Cursadas.Observaciones, Cursadas.DesSector, Cursadas.TipoCursada, Cursadas.Sector, " _
                    ...
                    + "Cursadas.OrdFecha <= '" + WHastaFecha + "' AND " _
                    + "Cursadas.TipoCursada >= 0 AND " _
                    + "Cursadas.TipoCursada <= 9999 AND " _
                    + "Cursadas.Sector >= " + Desde.Text + " AND " _
                    + "Cursadas.Sector <= " + Hasta.Text
    End Select
                
    Listado.Connect = Connect()
    Listado.ReportFileName = "WListaCursoSector.rpt"
    Listado.Action = 1
    
End Sub

Private Sub Cancela_click()
    PrgListaCursoLegajo.Hide
    Unload Me
    Menu.Show
End Sub
