...
Begin VB.Form PrgPagoNuevo 
   ...
   Begin VB.ListBox Pantalla 
      ...
      EndProperty
      Height          =   2010
      ItemData        =   "PagoNuevo.frx":0083
      Left            =   7200
      List            =   "PagoNuevo.frx":008A
      TabIndex        =   10
      Top             =   480
      Visible         =   0   'False
      Width           =   4575
   ...
   End
End
Attribute VB_Name = "PrgPagoNuevo"
...
Private WImpresion(15, 10) As String
Private WImpre2(15, 10) As String
Private WDebito(15, 2) As String
Private WCredito(15, 4) As String
Private WCuenta(15, 2) As String
            ...
            If Tipo1.Value = True Then
                ...
                If Trim(ZZZLetra) = "" And Val(ZZZTipo) = 0 And Val(ZZZPunto) = 0 And Val(ZZZNumero) = 0 Then
                    ...
                    If rstCtaCtePrv.RecordCount > 0 Then
                        ...
                        ZZTotal = rstCtaCtePrv!Total
                        ZZSaldo = rstCtaCtePrv!Saldo
                        rstCtaCtePrv.Close
                    End If
                    
                    Call Redondeo(ZZSaldo)
                    Call Redondeo(ZZTotal)
                    ...
                    If rstIvaComp.RecordCount > 0 Then
                        ...
                        If Val(WVector1.TextMatrix(iRow, 5)) = ZZTotal And Val(ZNroInterno) <> 0 Then
                            ZZSuma = rstIvaComp!Neto
                                Else
                            ...
                            ZPorce = Val(WVector1.TextMatrix(iRow, 5)) / ZTotal
                            ZZSuma = ZNeto * ZPorce
                        End If
                        rstIvaComp.Close
                            Else
                        ZZSuma = Val(WVector1.TextMatrix(iRow, 5)) / 1.21
                    End If
        ...
        If rstPagos.RecordCount > 0 Then
            Select Case Val(rstPagos!Tiporeg)
                Case 1
                    ...
                    ZImpoNeto = IIf(IsNull(rstPagos!ImpoNeto), "0", rstPagos!ImpoNeto)
                    WVector1.TextMatrix(Debito, 15) = Str$(ZImpoNeto)
                    ...
                    If ZImpoNeto = 0 Then
                        ...
                        ZSql = ZSql + " and CtaCtePrv.Letra = " + "'" + WVector1.TextMatrix(Debito, 2) + "'"
                        ZSql = ZSql + " and CtaCtePrv.Tipo = " + "'" + WVector1.TextMatrix(Debito, 1) + "'"
                        ZSql = ZSql + " and CtaCtePrv.Punto = " + "'" + WVector1.TextMatrix(Debito, 3) + "'"
                        ZSql = ZSql + " and CtaCtePrv.Numero = " + "'" + WVector1.TextMatrix(Debito, 4) + "'"
                        spCtaCtePrv = ZSql
                        Set rstCtaCtePrv = db.OpenRecordset(spCtaCtePrv, dbOpenSnapshot, dbSQLPassThrough)
                        If rstCtaCtePrv.RecordCount > 0 Then
                            ZNroInterno = Str$(rstCtaCtePrv!NroInterno)
                        ...
                        Set rstIvaComp = db.OpenRecordset(spIvaComp, dbOpenSnapshot, dbSQLPassThrough)
                        If rstIvaComp.RecordCount > 0 Then
                            If Val(WVector1.TextMatrix(Debito, 5)) = ZZTotal And Val(ZNroInterno) <> 0 Then
                                ZZSuma = rstIvaComp!Neto
                                    Else
                                ZNeto = rstIvaComp!Neto
                                ZIva = rstIvaComp!Iva21
                                ...
                                ZExento = rstIvaComp!Exento
                                ZTotal = ZNeto + ZIva + ZIva27 + ZIva105 + ZIb + ZIva5 + ZExento
                                ZPorce = Val(WVector1.TextMatrix(Debito, 5)) / ZTotal
                                ZZSuma = ZNeto * ZPorce
                            End If
                            rstIvaComp.Close
                                Else
                            ZZSuma = Val(WVector1.TextMatrix(Debito, 5)) / 1.21
                        End If
                        
                        WVector1.TextMatrix(Debito, 15) = Str$(ZZSuma)
                ...
                Case 2
                    ...
                    WVector1.TextMatrix(Credito, 13) = rstPagos!ClaveRecibo
                    ZCta = IIf(IsNull(rstPagos!Cuenta), "", rstPagos!Cuenta)
...
Sub Imprime_Datos()
    ...
    If RstProveedor.RecordCount > 0 Then
        ...
        WTipoIbCaba = IIf(IsNull(RstProveedor!CodIbCaba), "0", RstProveedor!CodIbCaba)
   Rem     WTipoIbCaba = RstProveedor!CodIbCaba
        RstProveedor.Close
        Call Format_datos
    End If
...
Private Sub cmdAdd_Click()
   ...
   If Proveedor.Text = "10167878480" Or Proveedor.Text = "10000000100" Or Proveedor.Text = "10071081483" Or Proveedor.Text = "10069345023" Or (Val(ZProvincia) = 24 And ZTipoProv = 1) Then
        ...
        If Debito = Credito Then
            ...
            For iRow = 1 To 15
                ...
                If Val(WVector1.TextMatrix(iRow, 5)) <> 0 Then
                    ...
                    XImpoNeto = WVector1.TextMatrix(iRow, 15)
                    ...
                    For Cicla = 1 To ZHasta
                        If CargaEmpresa(Cicla, 1) <> "" Then
                            ...
                            If WEntra = "S" Then
                                ...
                                If Proveedor.Text = "10167878480" Or Proveedor.Text = "10000000100" Or Proveedor.Text = "10071081483" Or Proveedor.Text = "10022098824" Or Proveedor.Text = "10069345023" Then
                                    ZSql = ""
...
Private Sub Command1_Click()
    ...
    For iRow = 1 To 15
        ...
        If Val(WVector1.TextMatrix(iRow, 12)) <> 0 Or Left$(WVector1.TextMatrix(iRow, 7), 2) <> "" Then
            ...
            XObservaciones2 = Left$(WVector1.TextMatrix(iRow, 11), 20)
            XImporte2 = WVector1.TextMatrix(iRow, 12)
            ClaveRecibos = WVector1.TextMatrix(iRow, 13)
            Cuit = WVector1.TextMatrix(iRow, 14)
            ClaveCtacte = WVector1.TextMatrix(iRow, 13)
            ...
            If Val(XTipo2) = 3 Then
                ...
                ZSql = ZSql + "Estado2 = " + "'" + "P" + "',"
                ZSql = ZSql + "Destino = " + "'" + "" + "'"
                ZSql = ZSql + " Where Clave = " + "'" + ClaveRecibos + "'"
                spRecibos = ZSql
                Set rstRecibos = db.OpenRecordset(spRecibos, dbOpenSnapshot, dbSQLPassThrough)
    ...
    If Tipo1.Value = True Then
        ...
        ZSql = ZSql + " Where Clave = " + "'" + ClaveCtaprv + "'"
        spCtaCtePrv = ZSql
        Set rstCtaCtePrv = db.OpenRecordset(spCtaCtePrv, dbOpenSnapshot, dbSQLPassThrough)
    ...
    Select Case XIndice
        ...
        Case 1
            If Tipo1.Value = True Then
                ...
                If Entra = "S" Then
                    ...
                    If Entra = "N" Then
                        m$ = "La cantidad de facturas a cancelar supera las 15"
                        A% = MsgBox(m$, 0, "Emision de Ordenes de Pago")
                        Exit Sub
                    End If
        ...
        Case 2
            ...
            If Entra = "S" Then
                ...
                If Entra = "N" Then
                    m$ = "La cantidad de valores entregados supera los 15"
                    A% = MsgBox(m$, 0, "Emision de Ordenes de Pago")
                    Exit Sub
                End If
...
Private Sub calcret_Click()
    ...
    Rem by nan 12-6-2014
    Rem Call ImpreretibCiudad
    Rem fin by nan
    ...
    Rem Retencion.Text = "982.33"
    Rem RetIb.Text = "122.23"
                ...
                Rem If XTBase >= 400 Then
                    ...
                    For iRow = 1 To 15
                        ...
                        If (Val(WVector1.TextMatrix(iRow, 1)) <> 0 And ZRechazado = 0) Or Tipo2.Value = True Then
                            ...
                            If WTipoiva = 2 Then
                                Rem If ZZSuma <> 0 Then
                                Rem     WImpre4 = ZZSuma
                                Rem         Else
                                Rem     WImpre4 = WImpre4 / 1.21
                                Rem End If
                                WImpre4 = ZZSuma
                            End If
                            ...
                            If ZFechaCompa >= "20071201" Then
                                ...
                                Select Case WTipoIb
                                    ...
                                    Case Else
                                End Select
                                WVector1.TextMatrix(iRow, 15) = XImpre4
                ...
                If Val(Wempresa) = 1 Then
                    ...
                    Rem If XTBase >= 500 Then
                        ...
                        For iRow = 1 To 15
                            ...
                            If rstIvaComp.RecordCount > 0 Then
                                ZRechazado = IIf(IsNull(rstIvaComp!Rechazado), "0", rstIvaComp!Rechazado)
                                If ZZSaldo = ZZTotal And Val(ZNroInterno) <> 0 Then
                                    ZZSuma = rstIvaComp!Neto
                                        Else
                                    ZZSuma = Val(WVector1.TextMatrix(iRow, 5)) / 1.21
                                End If
...
Private Sub Impreretib()
    ...
    For iRow = 1 To 15
        ...
        If (Val(WVector1.TextMatrix(iRow, 1)) <> 0 And ZRechazado = 0) Or Tipo2.Value = True Then
            ...
            ZSql = ZSql + "Select *"
            ZSql = ZSql + " FROM CtaCtePrv"
            ZSql = ZSql + " Where CtaCtePrv.Clave = " + "'" + ClaveCtaprv + "'"
            spCtaCtePrv = ZSql
            Set rstCtaCtePrv = db.OpenRecordset(spCtaCtePrv, dbOpenSnapshot, dbSQLPassThrough)
            If rstCtaCtePrv.RecordCount > 0 Then
                ZNroInterno = Str$(rstCtaCtePrv!NroInterno)
            ...
            If rstIvaComp.RecordCount > 0 Then
                ...
                If ZZSaldo = ZZTotal And Val(ZNroInterno) <> 0 Then
                    ZZSuma = rstIvaComp!Neto
                        Else
                    ZZSuma = rstIvaComp!Neto
                    Rem ZZSuma = Val(WVector1.TextMatrix(iRow, 5)) / 1.21
                End If
                rstIvaComp.Close
                    Else
                ZZSuma = Val(WVector1.TextMatrix(iRow, 5)) / 1.21
            ...
            WImpre4 = Val(WVector1.TextMatrix(iRow, 15))
            XImpre4 = Str$(WImpre4)
            ...
            With rstImpreRetIb
                ...
                If WPorceIbCaba <> 0 Then
                    !Categoria1 = "SUJETO A RETENCION " + Str$(WPorceIbCaba) + "%"
                        Else
                    If WTipoIbCaba = 3 Then
                        !Categoria1 = "SUJETO A RETENCION 3%"
                            Else
                        !Categoria1 = "SUJETO A RETENCION 3%"
                    End If
                End If
        ...
        If Val(ZCarpeta(Ciclo)) <> 0 Then
            ...
            For Cicla = 1 To ZHasta
                If CargaEmpresa(Cicla, 1) <> "" Then
                    ...
                    If WEntra = "S" Then
                        If Proveedor.Text <> "10167878480" And Proveedor.Text <> "10000000100" And Proveedor.Text <> "10071081483" And Proveedor.Text <> "10069345023" Then
        ...
        Case 9
            If Len(WVector1.Text) = 5 Then
                If Right$(WVector1.Text, 2) < 6 Then
                    WVector1.Text = WVector1.Text + "/2016"
                        Else
...
Private Sub Limpia_Vector()
    ...
    WVector1.Row = 0
    For Ciclo = 1 To WVector1.Cols - 4
        WVector1.Col = Ciclo
...
Private Sub Lectora_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        If Len(Lectora.Text) = 31 Then
                ...
                If ZZNroCuenta = "00000035626" Then
                    ...
                    If ZZNroCuenta = "00030521158" Then
                        ...
                        If ZZNroCuenta = "00830029723" Then
                            ...
                            If ZZNroCuenta = "05095506300" Then
