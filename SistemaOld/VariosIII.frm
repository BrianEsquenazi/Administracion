...
Begin VB.Form PrgVariosIII 
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Height          =   3855
      Left            =   360
      OleObjectBlob   =   "VariosIII.frx":0000
      TabIndex        =   3
      Top             =   1200
      Width           =   8655
   End
   ...
   Begin VB.ListBox Pantalla 
      Height          =   1815
      ItemData        =   "VariosIII.frx":09DE
      Left            =   120
      List            =   "VariosIII.frx":09E5
      TabIndex        =   1
      Top             =   6240
      Visible         =   0   'False
      Width           =   5295
   ...
   End
End
Attribute VB_Name = "PrgVariosIII"
...
Private Sub Graba_Click()
        ...
        If Credito.Value = False Then
            XTotal = Str$(WTotal)
            XTotalUs = Str$(WTotal / Val(WParidad.Text))
            XSaldo = Str$(WTotal)
            XSaldoUs = Str$(WTotal / Val(WParidad.Text))
            XNet = Str$(WNeto)
            XIva1 = Str$(WIva1)
            XIva2 = Str$(WIva2)
            XImpoIb = Str$(WImpoIb)
            ...
            XFlete = ""
                Else
            XTotal = Str$(WTotal * -1)
            XTotalUs = Str$(WTotal * -1 / Val(WParidad.Text))
            XSaldo = Str$(WTotal * -1)
            XSaldoUs = Str$(WTotal * -1 / Val(WParidad.Text))
            XNet = Str$(WNeto * -1)
            XIva1 = Str$(WIva1 * -1)
            XIva2 = Str$(WIva2 * -1)
    ...
    If KeyAscii = 13 Then
        ...
        If Existe = "S" Then
            ...
            Do
                With rstRecibos
                    ...
                    If rstRecibos.RecordCount > 0 Then
                        ...
                        Retencion = rstRecibos!Retganancias + rstRecibos!RetIva + rstRecibos!RetOtra + rstRecibos!Retencion
                    
                        If Val(rstRecibos!Renglon) = 1 Then
                        If Retencion > 0 Then
                            XCredito = XCredito + 1
                            Vector2(XCredito, 1) = rstRecibos!Tipo2
                            Vector2(XCredito, 2) = rstRecibos!Numero2
                            Vector2(XCredito, 3) = rstRecibos!Fecha
                            Vector2(XCredito, 4) = rstRecibos!Fecha
                            Vector2(XCredito, 5) = Retencion
                            Vector2(XCredito, 6) = rstRecibos!Clave
                        End If
                        End If
                        ...
                        Select Case Val(rstRecibos!Tiporeg)
                            Case 1
                                ...
                                Vector1(XDebito, 6) = rstRecibos!Clave
                            Case 2
                                XCredito = XCredito + 1
                                Vector2(XCredito, 1) = rstRecibos!Tipo2
                                Vector2(XCredito, 2) = rstRecibos!Numero2
                                WFecha2 = rstRecibos!Fecha2
                                If rstRecibos!Cliente = "G00065" And Val(rstRecibos!Tipo2) = 2 Then
                                    ...
                                    WFecha2 = XFec2
                                End If
                                Vector2(XCredito, 3) = WFecha2
                                If rstRecibos!FechaOrd > rstRecibos!fechaord2 Then
                                    Vector2(XCredito, 3) = rstRecibos!Fecha
                                End If
                                Vector2(XCredito, 4) = rstRecibos!Fecha
                                Vector2(XCredito, 5) = rstRecibos!Importe2
                                Vector2(XCredito, 6) = rstRecibos!Clave
                            Case Else
                        End Select
                        rstRecibos.Close
            ...
            For Cicla = 1 To XDebito
                ...
                WNumero = Vector1(Cicla, 4)
                WImporte = Vector1(Cicla, 5)
                WClave = Vector1(Cicla, 6)
        
                ClaveCtacte = WTipo + WNumero + "01"
                spCtacte = "ConsultaCtacte " + "'" + ClaveCtacte + "'"
                Set rstCtacte = db.OpenRecordset(spCtacte, dbOpenSnapshot, dbSQLPassThrough)
                If rstCtacte.RecordCount > 0 Then
                    Paridad = rstCtacte!Paridad
                    If Paridad = 0 Then
                        Paridad = 1
                    End If
                    rstCtacte.Close
                ...
                Dolares = WImporte / Paridad
                WSuma = WSuma + Dolares
                    
            Next Cicla
            
            For Cicla = 1 To XCredito
    
                WTipo = Vector2(Cicla, 1)
                WNumero = Vector2(Cicla, 2)
                WFecha2 = Vector2(Cicla, 3)
                WFecha = Vector2(Cicla, 4)
                WImporte = Vector2(Cicla, 5)
                WClave = Vector2(Cicla, 6)
            ...
            If WSuma >= 0 Then
                ...
                Auxi1 = WSuma / 1.21
                DBGrid1.Col = 1
                DBGrid1.Text = Pusing("##,###.##", Auxi1)
                DBGrid1.Row = 1
                DBGrid1.Col = 0
                DBGrid1.Text = "Recibo Nro.: " + Recibo.Text
                    Else
                Debito.Value = False
                Credito.Value = True
                DBGrid1.Col = 0
                DBGrid1.Text = "Notas de Credito por Diferencia de Cambio"
                Auxi1 = Abs(WSuma) / 1.21
                DBGrid1.Col = 1
                DBGrid1.Text = Pusing("##,###.##", Auxi1)
                DBGrid1.Row = 1
                DBGrid1.Col = 0
...
Private Sub Fecha_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        If Auxi = "S" Then
            ...
            Set rstCambios = db.OpenRecordset(spCambios, dbOpenSnapshot, dbSQLPassThrough)
            If rstCambios.RecordCount > 0 Then
                WParidad.Text = Pusing("###,###.##", Str$(rstCambios!Cambio))
                        Else
                WParidad.Text = ""
            End If
            If Val(WParidad.Text) <> 0 Then
                Call Calcula_FechaVto
                Vencimiento.Text = Wvencimiento
                Recibo.SetFocus
                    Else
...
Sub Impresion()
    ...
    For XX% = 1 To 2
        ...
        Impotot = Val(Total.Caption) / Val(WParidad.Text)
        ...
        If Val(ImpoIb.Caption) <> 0 Then
                Print #1, Tab(56); "Per.I.B.";
                Print #1, Tab(67); "$"; Alinea("###,###.##", ImpoIb.Caption)
                        Else
                If Val(Iva2.Caption) <> 0 Then
                    Print #1, Tab(61); "10.5";
                    Print #1, Tab(67); "$"; Alinea("###,###.##", Iva2.Caption)
                        Else
                    Print #1, ""
                End If
        End If
