...
Begin VB.Form PrgOrdenComplementoCertificado 
   BackColor       =   &H00C0FFFF&
   ...
   Begin VB.Frame IngresaObservaciones 
      Caption         =   "Ingreso de Certiifcados de Analisis"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "PrgOrdenComplementoCertificado"
...
Dim rstObservaOrdenCertificado As Recordset
Dim spObservaOrdenCertificado As String
...
Private Sub FinIngresaObserva_Click()
    ...
    ZSql = ""
    ZSql = ZSql + "DELETE ObservaOrdenCertificado"
    ZSql = ZSql + " Where Orden = " + "'" + Orden.Text + "'"
    spObservaOrdenCertificado = ZSql
    Set rstObservaOrdenCertificado = db.OpenRecordset(spObservaOrdenCertificado, dbOpenSnapshot, dbSQLPassThrough)
    
    ZGraba = "1"

    For Ciclo = 1 To 100
        If UCase(Trim(WVector1.TextMatrix(Ciclo, 2))) = "PENDIENTE" Then
            ZGraba = "0"
        End If
        If UCase(Trim(WVector1.TextMatrix(Ciclo, 2))) = "OK" Then
            ZSql = ZSql + "INSERT INTO ObservaOrdenCertificado ("
            ZSql = ZSql + "Orden ,"
            ZSql = ZSql + "Articulo )"
            ZSql = ZSql + "Values ("
    ...
    PrgOrdenComplementoCertificado.Hide
    Unload Me
...
Private Sub Form_Load()
        ...
        With rstOrden
            ...
            Do
                If .EOF = False Then
                    ...
                    Renglon = Renglon + 1
                    WVector1.TextMatrix(Renglon, 1) = rstOrden!Articulo
                    WVector1.TextMatrix(Renglon, 2) = "Pendiente"
    ...
    For Ciclo = 1 To Renglon
        ...
        ZSql = ""
        ZSql = ZSql + "Select *"
        ZSql = ZSql + " FROM ObservaOrdenCertificado"
        ZSql = ZSql + " Where Orden = " + "'" + Orden.Text + "'"
        ZSql = ZSql + " and Articulo = " + "'" + WVector1.TextMatrix(Ciclo, 1) + "'"
        spObservaOrdenCertificado = ZSql
        Set rstObservaOrdenCertificado = db.OpenRecordset(spObservaOrdenCertificado, dbOpenSnapshot, dbSQLPassThrough)
        If rstObservaOrdenCertificado.RecordCount > 0 Then
            WVector1.TextMatrix(Ciclo, 2) = "Ok"
            rstObservaOrdenCertificado.Close
        End If
