...
Begin VB.Form PrgModpedNuevo 
   ...
   Begin VB.ListBox Pantalla 
      ...
      EndProperty
      Height          =   2220
      ItemData        =   "modpednuevo.frx":0000
      Left            =   3360
      List            =   "modpednuevo.frx":0007
      TabIndex        =   1
      Top             =   5880
      Visible         =   0   'False
      Width           =   8055
...
Private Sub Form_Activate()
    ...
    If ZZPasaProcesoActualiza = "S" Then
        ...
        ZImpo1 = Val(WVector1.TextMatrix(WVector1.Row, 4))
        ZImpo2 = 0
        For ZZCiclo = 2 To 24 Step 2
            ZImpo2 = ZImpo2 + Val(XLote(ZZPasaFila, ZZCiclo))
        Next ZZCiclo
        ...
        If Val(A1) = Val(A2) Then
            WVector1.TextMatrix(WVector1.Row, 6) = "S"
        End If
        
        Rem If Val(WVector1.TextMatrix(WVector1.Row + 1, 3)) <> 0 Then
        Rem     WVector1.Row = ZZPasaFila - 1
        Rem     WVector1.Col = ZZPasaColumna + 1
        Rem         Else
        Rem     WVector1.Row = ZZPasaFila
        Rem     WVector1.Col = ZZPasaColumna
        Rem End If
...
Private Sub Graba_Click()
    ...
    For a = 1 To 99
        ...
        If Cantidad <> 0 Or Resta <> 0 Then
            ...
            ZZRenglon = ZZRenglon + 2
            
            If Val(XCantiLote4) <> 0 Or Val(XCantiLote5) <> 0 Or Val(XCantiLote6) <> 0 Then
                ZZRenglon = ZZRenglon + 1
            End If
            If Val(XCantiLote7) <> 0 Or Val(XCantiLote8) <> 0 Or Val(XCantiLote9) <> 0 Then
                ZZRenglon = ZZRenglon + 1
            End If
            If Val(XCantiLote10) <> 0 Or Val(XCantiLote11) <> 0 Or Val(XCantiLote12) <> 0 Then
                ZZRenglon = ZZRenglon + 1
            End If
        End If
    Next a
    
    If ZZRenglon > 17 Then
        m$ = "La cantidad de renglones que surge de esta actualizacion supera la capacidad del remito"
        a = MsgBox(m$, 0, "MODULO DE FACTURACION")
    End If
    ...
    If SumaEnvases = 0 Then
        ...
        For CicloEnvase = 1 To 100
            For XCicla = 81 To 92
                XPlastico = XPlastico + Val(XLote(CicloEnvase, XCicla))
            Next XCicla
        Next CicloEnvase
        
        If XPlastico <> 0 Then
            m$ = m$ + Str$(XPlastico) + " Pallets Plasticos " + Chr$(13) + ""
        End If
    ...
    For a = 1 To 99
        ...
        If Trim(Articulo) <> "" Then
            ...
            XPlastico = Val(XLote(WLugar, 81)) + Val(XLote(WLugar, 82)) + Val(XLote(WLugar, 83)) + Val(XLote(WLugar, 84)) + Val(XLote(WLugar, 85)) + Val(XLote(WLugar, 86)) + Val(XLote(WLugar, 87)) + Val(XLote(WLugar, 88)) + Val(XLote(WLugar, 89)) + Val(XLote(WLugar, 90)) + Val(XLote(WLugar, 91)) + Val(XLote(WLugar, 92))
            
            For XCicla = 31 To 53
                If XPlastico <> 0 Then
                    If Val(XLote(WLugar, XCicla)) = 0 Then
                        XLote(WLugar, XCicla) = "31"
                        XLote(WLugar, XCicla + 1) = Str$(XPlastico)
                        Exit For
                    End If
                End If
            Next XCicla
            ...
            Rem ZSql = ZSql & " MarcaFactura = " + "'" + Trim(Str$(MarcaFactura.ListIndex)) + "'"
            Rem ZSql = ZSql & " Where Pedido = " + "'" + Pedido.Text + "'"
            Rem spPedido = ZSql
            ...
            If WTipoPedido = "FA" Then
                ZSql = ""
                ZSql = ZSql & "UPDATE Pedido SET "
                ZSql = ZSql & " MarcaFactura = " + "'" + "9" + "'"
                ZSql = ZSql & " Where Pedido = " + "'" + Pedido.Text + "'"
                spPedido = ZSql
                Set rstPedido = db.OpenRecordset(spPedido, dbOpenSnapshot, dbSQLPassThrough)
                
                sTo = "ebiglieri@surfactan.com.ar; msosa@surfactan.com.ar"
                sCC = ""
                sBCC = ""
                sSubject = "Aviso para autorizar facturacion del pedido " + Pedido.Text
                sBody = "Aviso para autorizar facturacion del pedido " + Pedido.Text
